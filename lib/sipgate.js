...
module.exports = () => {
  ...
  module.settings.getSipgateIo = tokens => apiGet(tokens, 'settings/sipgateio');
  module.settings.setSipgateIoUrls = (tokens, url) => apiPut(tokens, 'settings/sipgateio', data => data, { incomingUrl: url, outgoingUrl: url, log: true });
  ...
  module.getCredentials = tokens => new Promise((resolve, reject) => {
    module.auth.getUserInfo(tokens).then((info) => {
      apiGet(tokens, `${info.sub}/devices`).then((result) => {
        resolve(result);
      });
    });
  });
